...
const PatientDetailsCardv2 = ({ tableData }) => {
  const patientInfo = {
    name: "Dr. Riya Sharma",
    image: "", // Add image URL here if you have one
    description: "General Practitioner at XYZ Hospital",
  };
              ...
              src={
                patientInfo.image
                  ? process.env.PUBLIC_URL +
                    `/assets/images/${patientInfo.image}`
                  : process.env.PUBLIC_URL + "/assets/images/default-male.jpg"
              }
              alt={patientInfo.name}
            />
            <div className="p-4">
              <h4 className="text-xl font-semibold mb-2">{patientInfo.name}</h4>
              <p className="text-gray-600 mb-4">
                {patientInfo.description || "No description available."}
              </p>
